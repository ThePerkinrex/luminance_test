...
fn main_loop(mut surface: GlfwSurface) {
	...
	let mut entity2 =
		engine::hud::Entity::new_entity_from_string(&mut surface, "Lies & deception".into(), &font)
			.unwrap();
	...
	let mut spatial_renderer = engine::spatial::Renderer::new(&mut surface, size);
	...
	'app: loop {
		...
		key_registry.for_pressed_keys(|key| {
			match key {
				Key::W => {
					pos[1] += 10;
					spatial_renderer.camera.pos.y += 0.2
				}
				Key::S => {
					pos[1] -= 10;
					spatial_renderer.camera.pos.y -= 0.2
				...
				Key::A => {
					pos[0] -= 10;
					spatial_renderer.camera.pos.x -= 0.2
				}
				Key::D => {
					pos[0] += 10;
					spatial_renderer.camera.pos.x += 0.2
		...
		spatial_renderer.camera.look_at(Point3::origin());
