...
pub fn tex_from_string<T: ToString, C: GraphicsContext>(surface: &mut C, name: T, font: &Font) -> Option<(Texture<Dim2, NormRGBA8UI>, [[u32; 2]; 4])> {
	let mut font_data = Vec::new();
	if let Ok(mut f) = std::fs::File::open(FONTS_PATH.join(font.name())) {
		...
		res_tex.upload_raw(GenMipmaps::No, &texels).expect("Error uploading texture");
		// 3 2
		// 0 1 (UV indices)
		return Some((res_tex, [
			[0,height as u32],
