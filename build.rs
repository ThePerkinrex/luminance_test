...
fn zip_dir<P: AsRef<Path>>(zip_writer: &mut ZipWriter<File>, dir_ref: P) -> IOResult<()> {
	...
	if dir.is_dir() {
		zip_writer.add_directory_from_path(dir.as_ref(), Default::default()).expect("Error adding dir to zip");
		for entry in read_dir(dir)? {
			...
			if path.is_dir() {
				...
				zip_writer.start_file_from_path(&path, Default::default()).expect("Error starting file to zip");
				let mut buf = Vec::new();
